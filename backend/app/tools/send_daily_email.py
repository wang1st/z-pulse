#!/usr/bin/env python3
"""
在容器/后端环境内手动发送指定日期的晨报邮件给已激活订阅者。

用法：
  python -m app.tools.send_daily_email 2025-12-18
  python -m app.tools.send_daily_email 2025-12-18 --email someone@example.com
  python -m app.tools.send_daily_email 2025-12-18 --dry-run
"""
...
# /app/backend/app/tools/send_daily_email.py -> project_root=/app
...
from ..services.email_service import send_daily_report  # noqa: E402
...
async def _send_all(report_date: date, only_email: str | None, dry_run: bool) -> int:
    ...
    try:
        ...
        # 解析报告JSON并重新渲染HTML，确保使用最新样式
        import json
        from ..services.report_render import render_daily_report_html, render_daily_report_pdf
        ...
        try:
            if report_json:
                ...
                pdf_bytes = render_daily_report_pdf(report_json, report_date.isoformat())
                pdf_filename = f"z-pulse-daily-{report_date.isoformat()}.pdf"
                logger.info(f"PDF生成成功: size={len(pdf_bytes)} bytes")
            else:
                logger.warning("报告JSON为空，跳过PDF生成")
                pdf_bytes = None
                pdf_filename = None
        except Exception as e:
        ...
        logger.info(
            f"准备发送日报邮件：date={report_date.isoformat()} subscribers={len(subscribers)} "
            f"dry_run={dry_run} only_email={only_email or '-'} pdf_attachment={pdf_filename is not None}"
        )
